// detail.js (anteriormente manga-detail.js)
...
// Validar token leyendo lista de usuarios
async function checkToken() {
    ...
    // Verificar token solicitando lista de usuarios
    try {
...
// Inicializar checkbox de favorito
...
function renderChapters(chapters, title, cid) {
    ...
    // Limpiar list excepto el control estático
    Array.from(container.children).forEach(child => {
        if (!child.classList.contains('mark-up-to')) container.removeChild(child);
    });

    // Ordenar capítulos numéricamente
    const sorted = [...chapters].sort((a, b) => parseInt(a.number, 10) - parseInt(b.number, 10));
    const maxNum = sorted.length ? parseInt(sorted[sorted.length - 1].number, 10) : 0;

    // Ajustar atributo max del input
...
// Descargar capítulo usando proxy y jsPDF
async function downloadChapter(chapterNumber, mangaTitle, url) {
    ...
    for (let i = 0; i < images.length; i++) {
        ...
        if (y + dims.height * scale > ph) { pdf.addPage(); y = 0; }
...
function createProgress() {
    ...
    Object.assign(c.style, {position:'fixed',top:'50%',left:'50%',transform:'translate(-50%,-50%)',background:'rgba(0,0,0,0.7)',padding:'20px',borderRadius:'10px',color:'#fff',textAlign:'center',zIndex:9999});
    const p = document.createElement('p'); p.innerText = 'Recogiendo páginas...';
    const bar = document.createElement('div'); Object.assign(bar.style, {width:'100%',height:'10px',background:'#ccc',borderRadius:'5px',marginTop:'10px'});
    const fill = document.createElement('div'); Object.assign(fill.style, {height:'100%',width:'0%',background:'#4caf50',borderRadius:'5px'});
    bar.append(fill); c.append(p, bar);
